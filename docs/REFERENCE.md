...
| 原始 NDJSON Worker 校验与 M2 用户文件导入 | `src/workers/ndjsonProtocol.ts`、`src/workers/ndjson.worker.ts`、`src/workers/browserImport.ts`、`src/application/browserInsightRuntime.ts`、`tests/worker/`、`tests/e2e/app.spec.ts` | [Web Workers API](https://developer.mozilla.org/en-US/docs/Web/API/Web_Workers_API)、[TextDecoder](https://developer.mozilla.org/en-US/docs/Web/API/TextDecoder) | 采用 transferable `ArrayBuffer`、fatal UTF-8、有限 ACK/backpressure、cancel/dispose、Worker receipt 与 Application 独立 hash 重算；bundled synthetic 与绑定活动 `ConsentGrant`/`RetentionPolicy` 的用户主动选择严格 `json-import` NDJSON v1 均已接入，确认后走 ImportSession 原子发布，Worker 不可用或 consent 失效时 fail closed。 |
...
- M5 原生壳基线由 `src-tauri/`、`src/nativeBridge.ts`、`src/ui/NativeShellControls.tsx` 与 `scripts/check-native-artifact.mjs` 提供。Gate 5 仍为 `NOT_RUN`：只有 workspace 内 typecheck/lint/build/IPC 单测/artifact 检查与**单机** NSIS+MSI bundle 存在性证据，缺跨机支持矩阵、资源测量和独立人工可访问性证据，也不存在任何已实跑的托管 CI 记录（`native_ipc` job 从未执行，见下条），不得据此宣称“已完成窄 Windows UIA 验证”。
...
- 原生 Rust 验证**不能**放到 Linux runner：`windows-rs` 声明 `#![cfg(windows)]`，非 Windows 上变空 crate 且被引用符号全部消失；`tauri` 在 Linux 还会拉入整套 GTK/WebKitGTK，需 `libgtk-3-dev`/`libwebkit2gtk-4.1-dev` 才能编译一个不跑任何 Linux 代码路径的测试二进制。因此 cargo IPC 单测由 `verify.yml` 的 `native_ipc` job 在 `windows-latest` 执行，Rust toolchain 由 `src-tauri/rust-toolchain.toml` 固定；gate 测试同时断言“Linux 命令注册表里不得出现 cargo/Cargo.toml”，防止有人误加到 pr/nightly/release 层级。Windows-only 依赖已收进 `[target.'cfg(windows)'.dependencies]`。
- **`native_ipc` job 从未实跑过**，不得把它当作已完成的托管 CI 证据。2026-09-18 实测：该 run `conclusion: failure` 且 `jobs=0`（`/actions/runs/<id>/jobs` 为空，github-actions check suite 的 `latest_check_runs_count: 0`），日志 404 且 `gh run rerun` 报 “cannot be retried” —— 这是 runner 分配之前就失败的特征，不是测试失败。`main` 分支上不含任何原生改动的提交同样是 `jobs=0`，两个分支近 100 次 run 无一启动 job，因此与原生 job 无关；转折点可定位到 2026-09-06 的 run `34055769223`。两条具体结论因此仍未证实：(1) “native crate 无法在 Linux 构建”来自本地 cargo 尝试，未被 CI 确认；(2) `cargo test` 在 `windows-latest` 上能否通过（含 MSVC 与 `rust-toolchain.toml` 能否在 runner 解析）未经验证 —— gate 测试只断言命令如何**注册**，不断言它**跑得起来**。处理方式：在 GitHub 网页端查看该 run 的启动报错（API 不返回，常见为 spending limit 或账户未验证），修好后确认 job 能到达 `Verify native IPC contract` 步骤。详见 [M5-NATIVE-SHELL.md](final/M5-NATIVE-SHELL.md)。
- 两条清除路径边界明确且不得互相替代：`clear_native_state` 只清原生壳（关 UIA、进 PRIVATE、推进 epoch），其 receipt 恒为 `native_records_cleared: 0`，因为原生壳不持有任何用户 payload；IndexedDB 用户数据由 `revokeConsent` 清除，走 fenced deletion journal 并靠 purge broadcast channel 做跨标签协调，且在 `AppShell.revokeReadonly` 中**先于** `clearNativeState` 执行。UI 已提供独立的「清理原生壳状态」入口并显式标注数据清除需走「撤回真实来源授权」，否则未授权任何真实来源的用户无法关闭 UIA 或推进 epoch。
