...
- 2026-09-09 的 Worker 与用户文件 ImportSession 接线由 `src/workers/browserImport.ts`、`src/workers/ndjson.worker.ts`、`src/application/browserInsightRuntime.ts`、`tests/worker/browserImport.test.ts` 与对应 Chromium E2E 断言提供；生产构建 identity 为 `021a9ac740c9e8e5111e5ee42e848a36f980afb544716d3c05e628033dbe2b1e`，bundled synthetic 与严格 `json-import` 本地文件路径已实测；持续桌面监听、自动采集和任意格式 adapter 仍未实现。
